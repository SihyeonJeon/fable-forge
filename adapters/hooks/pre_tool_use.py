...
Edits to `.wfb/` (authoring the spec) are always allowed.
"""
...
from common import (read_payload, project_root, run_gate, tool_name,  # noqa: E402
                    ...
                    under_wfb, is_spec_authoring, EDIT_TOOLS)
...
def _is_protected_state(p: str, root: str) -> bool:
    ...
    Canonical containment (not substring) so '.wfb/../x' or symlink aliases can't bypass."""
    return under_wfb(p, root) and not is_spec_authoring(p, root)
...
def main() -> int:
    ...
    # Exempt authoring the spec ONLY when every parsed edit target is a .wfb
    # artifact. A substring match on the whole command is gameable (a real-file edit
    # whose patch text merely mentions ".wfb/" would bypass), so require all paths.
    paths = edited_paths(payload)
    if paths:
        if any(_is_protected_state(p, root) for p in paths):
            sys.stderr.write(
                "wfb: editing gate state (.wfb/GRADE | ACTIVE | edits.txt | "
                "STATE | sessions) is not allowed — it would let an active task downgrade "
                "its own enforcement. Author .wfb/spec.json only.\n")
            return 2
        ...
        if spec_targets and len(spec_targets) != len(paths):
            ...
            sys.stderr.write(
                "wfb: do not edit .wfb/spec.json and implementation files in the "
        ...
        # real (non-.wfb) file present -> do NOT exempt; gate it
    elif ".wfb/" in edit_targets_blob(payload):
        return 0  # couldn't parse paths but references .wfb -> conservative allow
    ...
    pend = [p for p in paths if not under_wfb(p, root)]
    if pend:
        os.environ["WFB_PENDING"] = json.dumps(pend)
    rc, out = run_gate("validate", "--root", root, "--gate", "spec")
    if rc != 0:
        sys.stderr.write(
            "wfb: implementation blocked — SPEC gate not satisfied.\n"
            "Author .wfb/spec.json per the engineering procedure (restated_goal, "
